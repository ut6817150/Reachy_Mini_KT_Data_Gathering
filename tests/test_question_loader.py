...
from scripts.question_loader import QuestionFormatError, load_questions, parse_questions


def markdown_questions(count: int = 10) -> str:
    return "# Questions\n\n" + "\n\n".join(
        f"## Question {number}\n\nQuestion text {number}." for number in range(1, count + 1)
    )


class QuestionParserTests(unittest.TestCase):
    def test_parses_ten_sequential_questions(self) -> None:
        questions = parse_questions(markdown_questions())
        self.assertEqual(len(questions), 10)
        self.assertEqual(questions[0].number, 1)
        self.assertEqual(questions[-1].text, "Question text 10.")
    ...
    def test_loads_utf8_file(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            path.write_text(markdown_questions().replace("text 1", "text café 1"), encoding="utf-8")
            self.assertIn("café", load_questions(path)[0].text)
